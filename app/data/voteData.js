import 秧予1 from "../assets/秧予/秧予_IMG_1937.jpeg";
import 秧予3 from "../assets/秧予/秧予_IMG_2032.jpeg";
import 秧予5 from "../assets/秧予/秧予_IMG_2034.jpeg";
import 秧予6 from "../assets/秧予/秧予_IMG_1499.jpeg";
import 秧予7 from "../assets/秧予/秧予_IMG_1928.jpeg";
import 秧予9 from "../assets/秧予/秧予_IMG_1966.jpeg";
import 秧予10 from "../assets/秧予/秧予_IMG_2047.jpeg";
import 秧予11 from "../assets/秧予/秧予_IMG_0999.jpg";
import 秧予12 from "../assets/秧予/秧予_IMG_1799.jpeg";
import 秧予13 from "../assets/秧予/秧予_IMG_0998.jpg";
import 手槍Img from "../assets/手槍.jpg";
import 三角尺Img from "../assets/三角尺.jpg";
import 黑板Img from "../assets/黑板.jpg";
import 鎚子Img from "../assets/鎚子.jpg";
import 書Img from "../assets/書.jpg";
import 鍵盤Img from "../assets/鍵盤.jpg";
import 阿公阿嬤的禮物Img from "../assets/阿公阿嬤的禮物_秧予.jpg";
import 麥克風Img from "../assets/麥克風.jpg";
import 算盤Img from "../assets/算盤.jpg";
import 板手Img from "../assets/板手.jpg";
import 場記板Img from "../assets/場記板.jpg";
import 博士帽Img from "../assets/博士帽.jpg";
import 急救箱Img from "../assets/急救箱.jpg";
import 廚師帽Img from "../assets/廚師帽.jpg";
import 樂器Img from "../assets/樂器.jpg";
import 飛機Img from "../assets/飛機.jpg";
import 相機Img from "../assets/相機.jpg";
import 調色盤Img from "../assets/調色盤.jpg";
import 特斯拉Img from "../assets/特斯拉.jpg";
import VtuberImg from "../assets/Vtuber.jpg";
export const imageMap = {
  手槍: "/投票物件/手槍.mp4",
  三角尺: "/投票物件/三角尺.mp4",
  黑板: "/投票物件/黑板.mp4",
  鎚子: "/投票物件/鎚子.mp4",
  書: "/投票物件/書.mp4",
  鍵盤: "/投票物件/鍵盤.mp4",
  阿公阿嬤的禮物: "/投票物件/阿公阿嬤的禮物_秧予.mp4",
  麥克風: "/投票物件/麥克風.mp4",
  算盤: "/投票物件/算盤.mp4",
  板手: "/投票物件/板手.mp4",
  場記板: "/投票物件/場記板.mp4",
  博士帽: "/投票物件/博士帽.mp4",
  急救箱: "/投票物件/急救箱.mp4",
  廚師帽: "/投票物件/廚師帽.mp4",
  樂器: "/投票物件/樂器.mp4",
  飛機: "/投票物件/飛機.mp4",
  相機: "/投票物件/相機.mp4",
  調色盤: "/投票物件/調色盤.mp4",
  特斯拉: "/投票物件/特斯拉.mp4",
  Vtuber: "/投票物件/Vtuber.mp4",
};

// AI 圖片生成用的靜態圖（投票用 mp4，但 AI 生成需要 jpg）
export const aiImageMap = {
  手槍: 手槍Img,
  三角尺: 三角尺Img,
  黑板: 黑板Img,
  鎚子: 鎚子Img,
  書: 書Img,
  鍵盤: 鍵盤Img,
  阿公阿嬤的禮物: 阿公阿嬤的禮物Img,
  麥克風: 麥克風Img,
  算盤: 算盤Img,
  板手: 板手Img,
  場記板: 場記板Img,
  博士帽: 博士帽Img,
  急救箱: 急救箱Img,
  廚師帽: 廚師帽Img,
  樂器: 樂器Img,
  飛機: 飛機Img,
  相機: 相機Img,
  調色盤: 調色盤Img,
  特斯拉: 特斯拉Img,
  Vtuber: VtuberImg,
};

export const familyNames = [
  "五股阿公", "五股阿嬤", "北投阿公", "北投阿嬤",
  "乾阿公", "乾阿嬤", "小榆姑姑", "小莆叔叔",
  "彥廷舅舅", "大姑婆", "小姑婆", "五股姨婆",
  "蘆洲姨婆", "大叔公", "大嬸婆", "大欣欣姑姑", "昉昉姑姑",
  "阿暐叔叔", "美麗姑姑", "培涓阿北", "揚逸阿北",
  "惠瑩姑姑", "玉嬋姑姑", "自強阿北", "瑩芳姑姑",
  "曉茹阿姆", "涵涵姑姑", "雯雯姑姑", "依晨姊姊",
  "彤彤姊姊",
];

export const carouselItems = [
  { type: "video", src: "/秧予/秧予_吃1.mp4" },
  { type: "video", src: "/秧予/秧予_吃2.mp4" },
  { type: "video", src: "/秧予/秧予_公園1.mp4" },
  { type: "video", src: "/秧予/秧予_公園2.mp4" },
  { type: "video", src: "/秧予/秧予_睡1.mp4" },
  { type: "image", src: 秧予11 },
  { type: "image", src: 秧予12 },
  { type: "image", src: 秧予13 },
  { type: "image", src: 秧予1 },
  { type: "image", src: 秧予3 },
  { type: "image", src: 秧予5 },
  { type: "image", src: 秧予6 },
  { type: "image", src: 秧予7 },
  { type: "image", src: 秧予9 },
  { type: "image", src: 秧予10 },
];
